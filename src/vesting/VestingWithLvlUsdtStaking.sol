    ...
    ReentrancyGuardUpgradeable
{
    ...
    function getReservedAmount(address _user) external view returns (uint256) {
        ...
        return _userInfo.totalVestingAmount * _getReserveRate() / RESERVE_RATE_PRECISION;
    }
    ...
    function startVesting(uint256 _amount) external virtual nonReentrant {
        ...
        if (_totalVestingAmount * _getReserveRate() > _getStakedAmount(_sender) * RESERVE_RATE_PRECISION) {
    ...
    /**
     * @notice Return `reserveRate` of LVL-USDT tokens used to vest preLVL tokens.
     */
    function _getReserveRate() internal view returns (uint256) {
